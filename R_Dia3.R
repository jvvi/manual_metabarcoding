...
D1 <- ggplot(sfs_alphadiv, aes(x = ocean_depth, y = S.obs, fill =ocean_depth)) +
  geom_boxplot() +
  labs(title = NULL, y = "Richness", x ="Depth",  tag = "A") +
  scale_fill_manual(
...
D2 <- ggplot(sfs_alphadiv, aes(x=ocean_depth, y=S.chao1, fill =ocean_depth)) +
  geom_boxplot() +
  labs(title = NULL, y = "Chao1", x ="Depth",  tag = "B") +
  scale_fill_manual(
    values = c("30m" = "#17becf", "60m" = "#e46385")) +
  theme_bw() +
  theme(legend.position = "none")
D3 <- ggplot(sfs_alphadiv, aes(x=ocean_depth, y=S.evenness, fill =ocean_depth)) +
  geom_boxplot() +
  labs(title = NULL, y = "Evenness", x ="Depth",  tag = "C") +
  scale_fill_manual(
    values = c("30m" = "#17becf", "60m" = "#e46385")) +
  theme_bw() +
  theme(legend.position = "none")
D4 <- ggplot(sfs_alphadiv, aes(x=ocean_depth, y=S.shannon, fill =ocean_depth)) +
  geom_boxplot() +
  labs(title = NULL, y = "Shannon", x ="Depth", tag = "D") +
  scale_fill_manual(
    values = c("30m" = "#17becf", "60m" = "#e46385")) +
  theme_bw() +
  theme(legend.position = "none")
...
###Homocedasticidad------------
car::leveneTest(S.shannon ~ ocean_depth,  data = sfs_alphadiv) # Usamos Levene porque es más robusto frente a distribuciones de variables aleatroias no normales. F max se basa en el supuesto de normalidad
car::leveneTest(S.evenness ~ ocean_depth, data = sfs_alphadiv) # Usamos Levene porque es más robusto frente a distribuciones de variables aleatroias no normales. F max se basa en el supuesto de normalidad
car::leveneTest(S.obs ~ ocean_depth,      data = sfs_alphadiv) # Usamos Levene porque es más robusto frente a distribuciones de variables aleatroias no normales. F max se basa en el supuesto de normalidad
car::leveneTest(S.chao1 ~  ocean_depth,    data = sfs_alphadiv) # Usamos Levene porque es más robusto frente a distribuciones de variables aleatroias no normales. F max se basa en el supuesto de normalidad


##GLM calculos-----
glm_shannon <- glm(S.shannon ~ ocean_depth,
                   family = gaussian(link = identity), data = sfs_alphadiv)

glm_evenness <- betareg(S.evenness ~ ocean_depth,
                        data = sfs_alphadiv, link = "logit")

# Richness (Poisson or Negative Binomial regression)
# First check for overdispersion
poisson_model <- glm(S.obs ~ ocean_depth, 
...
if (summary(poisson_model)$dispersion > 1) {
  ...
  glm_richness <- glm.nb(S.obs ~ ocean_depth, data = sfs_alphadiv)
} else {
...
if (skewness(sfs_alphadiv$S.chao1) > 1) {
  # Use Gamma if skewed
  glm_chao1 <- glm(S.chao1 ~ ocean_depth,
...
mutate(ocean_depth = r_sfs_df$ocean_depth, 
...
test_pa <- adonis2(dist_sfs_pa ~ ocean_depth,  data=df_sfs, permutations = 1e3)
test_pa
...
# Poner nestedness en la mitad superior y turneover en inferirior
...
###------------PLOT combined beta----
#este grafico solo se ve descargandolo#
